...
colnames(data) <- c("Region", "Own_Resources", "Government_Assistance", "Column4", "Column5", "Column6", "Column7")

# Convert columns to numeric (if needed)
data[, 2:7] <- lapply(data[, 2:7], function(x) as.numeric(gsub(",", "", x)))  # Remove commas and convert to numeric

# Calculate total for Low SES based on columns 3 and 4
data$Low_SES_Total <- data$Own_Resources + data$Government_Assistance
...
# Create a bar plot for Low SES per region with manual color assignment
ggplot(data, aes(x = reorder(Region, -Low_SES_Total), y = Low_SES_Total, fill = Region)) +
  geom_bar(stat = "identity") +
  labs(title = "Low Socioeconomic Status (SES) by Region",
       x = "Region",
       y = "Total Low SES") +
