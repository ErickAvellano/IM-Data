...
colnames(data) <- c("Region", "Own_Resources", "Government_Assistance", "Column4", "Middle_SES_Column1", "Middle_SES_Column2", "Column7")

# Convert columns to numeric (if needed)
data[, 2:7] <- lapply(data[, 2:7], function(x) as.numeric(gsub(",", "", x)))  # Remove commas and convert to numeric

# Calculate total for Middle SES based on columns 5 and 6
data$Middle_SES_Total <- data$Middle_SES_Column1 + data$Middle_SES_Column2
...
# Create a bar plot for Middle SES per region with manual color assignment
ggplot(data, aes(x = reorder(Region, -Middle_SES_Total), y = Middle_SES_Total, fill = Region)) +
  geom_bar(stat = "identity") +
  labs(title = "Middle Socioeconomic Status (SES) by Region",
       x = "Region",
       y = "Total Middle SES") +
